...
final class AppFeatureTests: XCTestCase {
  func testDelete() async throws {
    ...
    await store.send(.path(.element(id: 0, action: .detail(.view(.deleteButtonTapped))))) {
  ...
  func testDetailEdit() async throws {
    ...
    await store.send(.path(.element(id: 0, action: .detail(.view(.editButtonTapped))))) {
    ...
    await store.send(.path(.element(id: 0, action: .detail(.view(.doneEditingButtonTapped))))) {
      $0.path[id: 0, case: \.detail]?.destination = nil
  ...
  func testRecording() async {
    ...
    await store.send(.path(.element(id: 1, action: .record(.view(.onTask)))))
