...
struct RecordMeeting {
  ...
  func handleAlert(state: inout State, action: Actions.Alert) -> EffectOf<Self> {
    switch action {
    case .confirmDiscard:
      return .run { _ in
        await self.dismiss()
  ...
  private func startSpeechRecognition(send: Send<Action>) async {
    do {
      ...
      for try await result in speechTask {
        await send(.effect(.speechResult(result)))
      }
    } catch {
      await send(.effect(.speechFailure))
    }
  }

  private func startTimer(send: Send<Action>) async {
    for await _ in self.clock.timer(interval: .seconds(1)) {
      await send(.effect(.timerTick))
    }
  }
}

@ViewAction(for: RecordMeeting.self)
...
extension AlertState where Action == RecordMeeting.Actions.Alert {
