...
final class VoiceMemosTests: XCTestCase {
  ...
  func testRecordAndPlayback() async throws {
    ...
    await store.receive(\.effect.recordPermissionResponse) {
    ...
    await store.send(.recordingMemo(.presented(.view(.onTask))))
    await store.send(.recordingMemo(.presented(.view(.stopButtonTapped)))) {
      $0.recordingMemo?.mode = .encoding
    }
    await store.receive(\.recordingMemo.effect.finalRecordingTime) {
      $0.recordingMemo?.duration = 2.5
    }
    await store.receive(\.recordingMemo.effect.audioRecorderDidFinish.success)
    ...
    await store.send(.voiceMemos(.element(id: deadbeefURL, action: .view(.playButtonTapped)))) {
    ...
    await store.receive(\.voiceMemos[id:deadbeefURL].effect.timerUpdated) {
    ...
    await store.receive(\.voiceMemos[id:deadbeefURL].effect.audioPlayerClient.success) {
  ...
  func testRecordMemoHappyPath() async throws {
    ...
    let recordingMemoTask = await store.send(.recordingMemo(.presented(.view(.onTask))))
    await self.clock.advance(by: .seconds(1))
    await store.receive(\.recordingMemo.effect.timerUpdated) {
  ...
  func testPermissionDenied() async {
    ...
    await store.send(.view(.openSettingsButtonTapped)).finish()
  ...
  func testRecordMemoFailure() async {
    ...
    await store.receive(\.recordingMemo.effect.audioRecorderDidFinish.failure)
  ...
  func testPlayMemoHappyPath() async {
    ...
    await store.send(.voiceMemos(.element(id: url, action: .view(.playButtonTapped)))) {
      $0.voiceMemos[id: url]?.mode = .playing(progress: 0)
    }
    await store.receive(\.voiceMemos[id:url].delegate.playbackStarted)
    await self.clock.advance(by: .milliseconds(500))
    await store.receive(\.voiceMemos[id:url].effect.timerUpdated) {
    ...
    await store.receive(\.voiceMemos[id:url].effect.audioPlayerClient.success) {
  ...
  func testPlayMemoFailure() async {
    ...
    let task = await store.send(.voiceMemos(.element(id: url, action: .view(.playButtonTapped)))) {
      $0.voiceMemos[id: url]?.mode = .playing(progress: 0)
    }
    await store.receive(\.voiceMemos[id:url].delegate.playbackStarted)
    await store.receive(\.voiceMemos[id:url].effect.audioPlayerClient.failure) {
      $0.voiceMemos[id: url]?.mode = .notPlaying
    }
  ...
  func testDeleteMemo() async {
    ...
    await store.send(.view(.onDelete([0]))) {
      $0.voiceMemos = []
    }
  }
  ...
  func testDeleteMemos() async {
    ...
    await store.send(.view(.onDelete([1]))) {
      $0.voiceMemos = [
        VoiceMemo.State(
          date: date,
          duration: 30,
