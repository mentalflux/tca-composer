...
@ComposeReducer(
  .bindable,
  children: [
    .identifiedArray("todos", of: Todo.self)
  ]
)
@Composer
struct Todos {
  ...
  func effect(state: inout State, action: EffectAction) -> EffectOf<Todos> {
    switch action {
    case .sortCompletedTodos:
  ...
  @ComposeBody(identifiedAction: \Action.Cases.todos, elementAction: \.binding)
  func resortTodos(action: BindingAction<Todo.State>) -> EffectOf<Self> {
    switch action {
    case \.isComplete:
      return .run { send in
        try await self.clock.sleep(for: .seconds(1))
        await send(.effect(.sortCompletedTodos), animation: .default)
      }
  ...
  func view(state: inout State, action: ViewAction) -> EffectOf<Self> {
    ...
    case var .move(source, destination):
      ...
      return .run { send in
        ...
        await send(.effect(.sortCompletedTodos))
      }
    }
  }
}

@ViewAction(for: Todos.self)
struct AppView: View {
      ...
      .navigationBarItems(
        trailing: HStack(spacing: 20) {
          ...
          Button("Clear Completed") {
            send(.clearCompletedButtonTapped, animation: .default)
          }
          .disabled(!store.todos.contains(where: \.isComplete))
          Button("Add Todo") { send(.addTodoButtonTapped, animation: .default) }
        }
      )
